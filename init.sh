...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  if [ "$REVERSE_PROXY_MODE" = 'grpcwebproxy' ]; then
    wget -c ${GH_PROXY}https://github.com/guoshang105/arnznewfile/releases/download/grpcwebproxy/grpcwebproxy-linux-$ARCH.tar.gz -qO- | tar xz -C $WORK_DIR
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/guoshang105/arnznewfile/raw/main/sqlite.db
